    ...
    public class AISight : AIBase
    {
        ...
        private void Update()
        {
            ...
            foreach (var actor in _oldVisible)
            {
                if (isDebug)
                {
                    Debug.LogError(actor.gameObject.name);
                    Debug.LogError(actor.GetViewDistance(Distance, _isAlerted));
                    Debug.LogError(AIUtil.IsInSight(_actor, actor.TopPosition, actor.GetViewDistance(Distance, _isAlerted), FieldOfView));
                }
